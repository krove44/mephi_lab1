...
size_t wchar_get_next_size(const void* data) {
    return sizeof(wchar_t);
}

int wchar_compare(const void* a, const void* b) {
    return wcscmp((const wchar_t*)a, (const wchar_t*)b);
}

size_t wchar_length(const void* a) {
    return wcslen((const wchar_t*)a);
}

void* wchar_clone(const void* data, size_t l) {
...
void wchar_free(void* data) {
    if (data) {
        free(data);
    }
}

bool wchar_is_delim(const void* data) {
